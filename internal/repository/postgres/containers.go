...
func (repo *PostgresRepository) GetContainerById(ctx context.Context, id int64) (*dto.ContainerDbo, error) {
	rows, err := pu.Dispatch[dto.ContainerDbo](
		...
		repo.db,
		`
		SELECT b.id, b.container_name, b.port, b.container_id, b.bot_id, b.project_id, b.user_id, b.name, b.description, b.icon, b.state
...
func (repo *PostgresRepository) GetAllBots(ctx context.Context) ([]dto.ContainerDbo, error) {
	rows, err := pu.Dispatch[dto.ContainerDbo](
		...
		repo.db,
		`
		SELECT b.id, b.container_name, b.port, b.bot_id, b.container_id, b.project_id, b.user_id, b.name, b.description, b.icon, b.state
		FROM bot_containers b
...
func (repo *PostgresRepository) CreateBot(ctx context.Context, bot dto.ContainerDbo) (int64, error) {
	rows, err := pu.Dispatch[dto.ContainerDbo](
		...
		repo.db,
		`
		INSERT INTO bot_containers (container_name, port, container_id, bot_id, project_id, user_id, name, description, icon, state)
		VALUES ($1::text, $2::integer, $3::text, $4::bigint, $5::bigint, $6::bigint, $7::text, $8::text, $9::text, $10::text)
		RETURNING *;
		`,
		bot.ContainerName,
		bot.Port,
